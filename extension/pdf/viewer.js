...
var Preferences = {
  ...
  initialize: function preferencesInitialize() {
    return (this.initializedPromise = this._readFromStorage(DEFAULT_PREFERENCES).then(
      function (prefObj) {
    ...
    mouseScroll: function PDFPresentationMode_mouseScroll(delta) {
      ...
      if (Math.abs(this.mouseScrollDelta) >= PAGE_SWITCH_THRESHOLD) {
        ...
        if (
          ...
          (page === this.pdfViewer.pagesCount && pageSwitchDirection === PageSwitchDirection.DOWN)
        ) {
          return;
        }
...
var TextLayerBuilder = (function TextLayerBuilderClosure() {
  ...
  TextLayerBuilder.prototype = {
    ...
    appendText: function TextLayerBuilder_appendText(geom, styles, ctx) {
      ...
      // Only build font string and set to context if different from last.
      if (fontHeight !== ctx.lastFontHeight || style.fontFamily !== ctx.lastFontFamily) {
      ...
      geom.textScale = (geom.vertical ? geom.height : geom.width) / (geom.canvasWidth / this.viewport.scale);
    ...
    compileTextContent: function TextLayerBuilder_compileTextContent() {
      ...
      var rotate = (360 + (Math.atan2(this.viewport.transform[1], this.viewport.transform[0]) / Math.PI) * 180) % 360;
      ...
      for (i = dontDoPadding ? len : 0; i < len; i++) {
        ...
        var farRight = geom.right !== null ? getDivPosition(textItems[geom.right]).left : bounds.right;
        var farBottom = geom.bottom !== null ? getDivPosition(textItems[geom.bottom]).top : bounds.bottom;

        // Update Padding. Apply textScale as appropriate (horizontal only).
        divi.style.paddingRight = Math.abs(farRight - right) / geom.textScale + "px";
        ...
        if (geom.left === null) {
          ...
          if (
            leftItem.right !== null &&
            leftItem.right !== i &&
            (leftItem.top === null || diffY(getDivPosition(leftItem).top, top) <= 0) &&
            (leftItem.bottom === null || diffY(getDivPosition(textItems[leftItem.bottom]).top, bottom) >= 0)
          ) {
...
function webViewerInitialized() {
  ...
  document.getElementById("viewDiigoAnnotation").addEventListener("click", function () {
...
window.addEventListener(
  ...
  function scalechange(evt) {
    ...
    var predefinedValueFound = selectScaleOption(evt.presetValue || "" + evt.scale);
...
$(document).ready(function () {
  ...
  function downloadWithAnnotations2() {
    ...
    PDFView.pdfDocument.getData().then(function (data) {
      var blob = PDFJS.createBlob(data, {
