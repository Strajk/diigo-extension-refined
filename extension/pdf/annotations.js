...
var diigoAnnotationHelper = (function diigoAnnotationHelper() {
  ...
  diigoAnnotationHelper.parseRange = function parseRange(pageIndex, rg) {
    ...
    if (d_list.length == 0) {
      return {
        rects: [
          [rects[0].left - offsetX, rects[0].top - offsetY, rects[0].left - offsetX + rects[0].width, rects[0].top - offsetY + rects[0].height],
        ],
        text: p.innerText,
      };
    }
  ...
  diigoAnnotationHelper.addLink = function addLink(ann) {
    ...
    if (ann.type == 9) {
      ...
      if (elem) {
        $(
          '<a href="#" data-dest="' + dest + '" title="' + ann.content + '" class="highlight-link ' + color + '">' + ann.content + "</a>"
        ).insertBefore(elem);
  ...
  $("#screenshot").bind("click", function (e) {
    ...
    if ($("div#diigo-chrome-installed").length > 0) {
      ...
      if (window.navigator.userAgent.match(/firefox/i)) {
        $("#popup-message")
          .html('Get <a href="https://www.diigo.com/tools/toolbar?client=ff">diigo firefox toolbar</a> to take capture.')
          .removeClass("hidden");
      } else {
        $("#popup-message")
          .html(
