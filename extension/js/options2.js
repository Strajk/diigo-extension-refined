...
function buildSelect() {
  for (var b = $("<select></select>"), d = 48; d < 91; d++)
    if (!(d > 57 && d < 65)) {
  ...
  b.on("change", function () {
    var g = $(this).parents(".shortcut").attr("data-type"),
      f = {};
    f["prefs.shortcut" + g + "Key"] = $(this).val();
    Prefs.set(f);
  });
  b.appendTo($(".shortcut"));
  $(".shortcut[data-type=Bookmark]")
    .find("select")
    .val(Prefs.get("prefs.shortcutBookmarkKey") || "Q");
  $(".shortcut[data-type=Annotate]")
    .find("select")
    .val(Prefs.get("prefs.shortcutAnnotateKey") || "D");
  $(".shortcut[data-type=Readlater]")
    .find("select")
    .val(Prefs.get("prefs.shortcutReadlaterKey") || "R");
  $(".shortcut[data-type=OpenOutlinerSidebar]")
    .find("select")
    .val(Prefs.get("prefs.shortcutOpenOutlinerSidebarKey") || "O");
  $(".shortcut[data-type=AnnotateArticle]")
    .find("select")
    .val(Prefs.get("prefs.shortcutAnnotateArticleKey") || "T");
  $(".shortcut[data-type=AnnotatePDF]")
    .find("select")
    .val(Prefs.get("prefs.shortcutAnnotatePDFKey") || "P");
  $(".shortcut[data-type=AnnotateScreenshot]")
    .find("select")
    .val(Prefs.get("prefs.shortcutAnnotateScreenshotKey") || "S");
}

function shortcutSwitchState(b) {
  $("#check-shortcut-" + b).is(":checked")
    ? $("#check-shortcut-" + b + "-ctrl,#check-shortcut-" + b + "-alt,#shortcut-" + b + " select").removeAttr("disabled")
    : $("#check-shortcut-" + b + "-ctrl,#check-shortcut-" + b + "-alt,#shortcut-" + b + " select").attr("disabled", "true");
}

function initFocusResearch() {
  updateLists(
    {
      list: Bg.GlobalData.myBmLists,
      outliners: Bg.GlobalData.outliners,
    },
    false
  );
  ...
  $("#diigobm-list-addBtn").on("click", function () {
    ...
    if (!$(this).parent().hasClass("processing"))
      if (Bg.GlobalData.permissions.createOutliner) {
        var b = $("#diigobm-list-addInput").val(),
          d = $("#diigobm-list-add .diigo-alert-tip"),
          e = [],
          g = Bg.GlobalData.myBmLists.length,
          f;
        for (f = 0; f < g; f++) e.push(Bg.GlobalData.myBmLists[f].title);
        if (b.match(/^\s*$/)) {
          d.show().find("span").text("Input a name");
          $("#diigobm-list-addInput").focus();
        } else if ($.inArray(b, e) !== -1) d.show().find("span").text("Name existed !");
        else {
          $(this).parent().addClass("processing");
          Bg.WebAPI.createList(b, function (c) {
            updateLists({
              list: c.lists,
              outliners: c.outliners,
            });
            c = c.newOutliner;
            var a = $("#diigobm-list");
  ...
  $("#w-upgrade").on("click", function (b) {
    b.preventDefault();
    googleEvent("Chrome extension", "Click upgrade from outliner creation", "outliner");
    chrome.tabs.create({
      url: "https://www.diigo.com/premium?f=outliner",
    });
  ...
  chrome.storage.local.get(["researchMode", "researchModeData"], function (b) {
    ...
    if (b.researchModeData) {
      ...
      if (b.group) {
        ...
        e.find(".content").text(d).attr("data-id", b.group).end().addClass("inputed");
      }
    }
  });
}
...
function perfs2UI() {
  ...
  $("#check-shortcut-a-article-ctrl").attr({
    checked: Prefs.get("prefs.shortcutAnnotateArticleCtrl") == "true",
  });
  $("#check-shortcut-a-article-alt").attr({
    checked: Prefs.get("prefs.shortcutAnnotateArticleAlt") == "true",
  });
  $("#check-shortcut-a-article").attr({
    checked: Prefs.get("prefs.shortcutAnnotateArticle") == "true",
  });
  $("#check-shortcut-a-pdf-ctrl").attr({
    checked: Prefs.get("prefs.shortcutAnnotatePDFCtrl") == "true",
  });
  $("#check-shortcut-a-pdf-alt").attr({
    checked: Prefs.get("prefs.shortcutAnnotatePDFAlt") == "true",
  });
  $("#check-shortcut-a-pdf").attr({
    checked: Prefs.get("prefs.shortcutAnnotatePDF") == "true",
  });
  $("#check-shortcut-a-screenshot-ctrl").attr({
    checked: Prefs.get("prefs.shortcutAnnotateScreenshotCtrl") == "true",
  });
  $("#check-shortcut-a-screenshot-alt").attr({
    checked: Prefs.get("prefs.shortcutAnnotateScreenshotAlt") == "true",
  });
  $("#check-shortcut-a-screenshot").attr({
    checked: Prefs.get("prefs.shortcutAnnotateScreenshot") == "true",
  });
  ...
  $("#check-shortcut-openOutlinerSidebar").attr({
    checked: Prefs.get("prefs.shortcutOpenOutlinerSidebar") == "true",
  });
  $("#check-shortcut-openOutlinerSidebar-ctrl").attr({
    checked: Prefs.get("prefs.shortcutOpenOutlinerSidebarCtrl") == "true",
  });
  $("#check-shortcut-openOutlinerSidebar-alt").attr({
    checked: Prefs.get("prefs.shortcutOpenOutlinerSidebarAlt") == "true",
  });
  ...
  $("#check-showVideoButton").attr({
    checked: Prefs.get("prefs.showVideoCapture") == "true",
  });
...
function updateLists(b, d) {
  ...
  var e = b.list,
    g = b.outliners,
    f = $("#diigobm-list").find(".item-container");
  f.empty();
  if (g.length) {
  ...
  if (e.length) {
    ...
    forEach(e, function (a) {
      var h = inList(a.id) ? a.title + " (added)" : a.title;
      $('<div class="menu-item" data-id="' + a.id + '">' + h + "</div>").appendTo(f);
    });
  }
...
function updateGroups(b, d) {
  ...
  if (g.length) {
    ...
    forEach(g, function (a) {
      ...
      $('<div class="menu-item" data-id="' + a.name + '">' + h + "</div>")
        .on("click", function () {
          console.log("Click");
          var i = a.name;
          if (popCtx.groupTagsDict[i] != undefined) showTags("group", popCtx.groupTagsDict[i]);
          else {
            $("#diigobm-group-tag").show();
            getSelectedTab(function (j) {
              chrome.tabs.sendMessage(j.id, {
      ...
      d
        ...
        : c.append(
            Utils.dom.buildOne(
              ...
              [a.displayName + (inGroup(a.name) ? " (shared)" : "")]
            )
          );
    });
    c.append(
  ...
  c.change(function () {
    ...
    if (a != 0 && a != -1 && a != -2 && a != -3) {
      ...
      console.log(popCtx);
      if (popCtx.groupTagsDict[a] != undefined) showTags("group", popCtx.groupTagsDict[a]);
      else {
        $("#diigobm-group-tag").show();
        chrome.tabs.getSelected(null, function (h) {
          chrome.tabs.sendMessage(h.id, {
...
function saveFocusResearchData() {
  ...
  saveTimer = setTimeout(function () {
    ...
    var d = $("#f-private").prop("checked"),
      e = $("#f-read-later").prop("checked"),
      g = $("#f-cache").prop("checked"),
      f = $("#diigobm-list").find(".content").attr("data-id"),
...
$("label input[data-pref]").on("change", function () {
  var b = $(this).attr("data-pref"),
    d = $(this).prop("checked");
  if (b == "prefs.directlyShowSearchResults")
    if (d == true) {
