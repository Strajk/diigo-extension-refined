...
function scrollNext() {
  ...
  if (isSelected) {
    ...
    if (w <= clientW && h > clientH) {
      ...
      if (y + h < prevScrollTop + 2 * clientH) document.body.scrollTop = y + h - clientH;
      else if (y + h > prevScrollTop + 2 * clientH) document.body.scrollTop = prevScrollTop + clientH;
    }
    //scroll - x:yes, y:no
    if (w > clientW && h <= clientH) {
      ...
      if (x + w < prevScrollLeft + 2 * clientW) document.body.scrollLeft = x + w - clientW;
      else if (x + w > prevScrollLeft + 2 * clientW) document.body.scrollLeft = prevScrollLeft + clientW;
    }
    //scroll - x:yes, y:yes
    if (w > clientW && h > clientH) {
      if (y + h == prevScrollTop + clientH) {
