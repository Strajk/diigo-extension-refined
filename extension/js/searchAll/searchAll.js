var SearchUrl = [],
  ...
  util = {
    contextmenus: function (a) {
      switch (a.menuItemId) {
        ...
        case 4:
          ...
          b =
            localStorage.googlelocal == "true"
              ? "http://" + chrome.i18n.getMessage("google") + "/search?q=" + b + "+" + encodeURIComponent(a.selectionText)
              : "http://www.google.com/search?q=" + b + "+" + encodeURIComponent(a.selectionText);
          break;
        case 3:
    ...
    GetAmazonUrl: function (a) {
      return chrome.i18n.getMessage("amazon") == "www.amazon.com"
        ? "http://www.amazon.com/gp/search?ie=UTF8&keywords=" +
            encodeURIComponent(a) +
            "&tag=diigo0c-20&index=aps&linkCode=ur2&camp=1789&creative=9325"
        : "http://go.redirectingat.com?id=18202X786788&xs=1&url=" +
            encodeURIComponent("http://" + chrome.i18n.getMessage("amazon") + "/s/field-keywords=" + encodeURIComponent(a));
    },
    GetEbayUrl: function (a) {
      chrome.i18n.getMessage("ebay");
      return (
        "http://go.redirectingat.com?id=18202X786788&xs=1&url=" +
        encodeURIComponent("http://" + chrome.i18n.getMessage("ebay") + "/sch/i.html?_sacat=See-All-Categories&_nkw=" + encodeURIComponent(a))
      );
    },
    GetGoogleUrl: function (a) {
      return localStorage.googlelocal == "true"
        ? "http://" + chrome.i18n.getMessage("google") + "/search?q=" + encodeURIComponent(a)
        : "http://www.google.com/search?q=" + encodeURIComponent(a);
    },
  ...
  OPTIONS_TRUE = ["switchbar", "searchsite", "superfish", "bookmarks", "history"],
  OPTIONS_FALSE = ["googlelocal", "searchselection"];
...
chrome.extension.onRequest.addListener(function (a, b, c) {
  switch (a.action) {
    case "getoption":
      c({
        option: JSON.stringify(localStorage),
      });
      break;
    case "search":
    ...
    case "newtip":
      if (!localStorage.version || localStorage.version != currentversion) {
        ...
        count = 1;
        window.open("https://www.diigo.com/searchAll/new-for-search-all.html?v=" + currentversion);
    ...
    case "getAD":
      $.get("http://api.hostip.info/get_json.php", function (e) {
        e = e.ip;
        var h = encodeURIComponent(a.url),
          m = a.query.replace(/\s/g, "+"),
          l = encodeURI(window.navigator.userAgent);
...
function getSearchString(a) {
  ...
  if (a.length > 0) {
    ...
    for (var f = 0; f < a.length; f++) {
      var g = a[f].getAttribute("name"),
        d = a[f].getAttribute("value");
      c = f == 0 ? c + g + "=" + d : c + "&" + g + "=" + d;
    }
  } else c = c.replace(/searchTerms/, "%s");
  c = c.replace(/searchTerms/, "%s");
  console.log(c);
  console.log(Opensearch);
  c &&
    LoadAllItems(function (e, h) {
      console.log(h.rows.length);
      console.log(h.rows.item(0));
      for (var m = 0; m < h.rows.length; m++)
        if (
          h.rows
            .item(m)
            .searchurl.toString()
            .match(/:\/\/([\w | .]+)/)[1] == c.match(/:\/\/([\w | .]+)/)[1]
        )
          return;
      ...
      chrome.extension.onRequest.addListener(function (l, n, j) {
        l.action == "getopensearchurl" &&
          j({
            searchurl: Opensearch[l.tabid][0],
            searchname: Opensearch[l.tabid][1],
          });
      });
    });
}
...
function newTab(a) {
  var b = localStorage.imagePreview,
    c = b.indexOf(",");
  b =
    'var form = document.createElement("form");form.setAttribute("method","POST");form.setAttribute("action","http://www.google.com/searchbyimage/upload");form.setAttribute("enctype","multipart/form-data");function addInput(key,value){  var input = document.createElement("input");  input.setAttribute("type","hidden");  input.setAttribute("name",key);  input.setAttribute("value",value);  form.appendChild(input);};addInput("image_content","' +
    b
      .substring(c + 1)
      .replace(/\+/g, "-")
      .replace(/\//g, "_")
...
function SearchImage(a) {
  ...
  if (a.indexOf("data:") == 0) {
    ...
    if (
      !lower_src.indexOf("data:image/bmp;") ||
      !lower_src.indexOf("data:image/gif;") ||
      !lower_src.indexOf("data:image/jpeg;") ||
      !lower_src.indexOf("data:image/jpg;") ||
      !lower_src.indexOf("data:image/png;") ||
      !lower_src.indexOf("data:image/tiff;") ||
      !lower_src.indexOf("data:image/x-ico;") ||
      !lower_src.indexOf("data:image/x-tiff;")
    ) {
      var b = a.indexOf(",");
      if (b != -1 && validateBase64(a.substring(b + 1))) {
        ...
        chrome.tabs.create(
          {
            url: "http://images.google.com/imghp?sbi=1",
            selected: true,
          },
          newTab
        );
