...
var TEXT = "",
  ...
  superfishBlacklist = ["www.interflora.co.uk"],
  Search = {
    ...
    showbar: function (a, c, e, j, g) {
      if (c == "special") c = Search.specialurl(e);
      else {
        c = RegExp("[&|?|#]" + c + "(.*?)(&|$|#)", "i");
      ...
      if (c) {
        ...
        $("body").prepend(
          '<div id="searchbar" class="searchbar"><div id="searchHistory">Keywords</div><ul id="searchHistory-list"></ul><div id="tab"><ul></ul><span class="button" title="Customize search engines"></span></div><div class="clear" style="clear:both;"></div></div>'
        );
        ...
        for (d = 0; d < j; d++) {
          ...
          g =
            ...
            '><a href="' +
            k +
            '" onClick="return false;" target="_parent"><img src="' +
            imgurl +
            '" />' +
            a[d].name +
            "</a></li>";
        ...
        $("#searchbar .button").click(function () {
          var h = chrome.extension.getURL("options.html");
    ...
    showsearchinsite: function (a) {
      ...
      a =
        '<div id="searchinsite"><div id="hidetool" style="display:none;"><span></span></div><div id="showtool"><h3>SearchO Extension<span class="hide"></span><span class="button"></span></h3><div class="searchbox">Search <b contentEditable="true">' +
        decodeURIComponent(a) +
        "</b> in this site<span></span></div>";
      $("body").append(a);
      $("#searchinsite #showtool .searchbox span").click(function () {
        var e =
          "http://www.searcho.com/search/google?q=" +
          c +
          "+" +
          encodeURIComponent($("#searchinsite #showtool .searchbox b").html()) +
          "&c=chromesearcho";
        window.open(e);
      });
    ...
    modifysearchosite: function () {
      OPTION.switchbar == "false" ? $("#searchBar #tab li span.popup").show() : $("#searchBar li a").removeAttr("target");
    },
    searchforselect: function () {
      $("#selectsearch").length > 0 ||
        chrome.extension.sendRequest(
          ...
          function (a) {
            function c(g, d) {
              if (g == "http://option/") window.open(chrome.extension.getURL("options.html"));
              else {
            ...
            a = a.searchs;
            $("body").prepend(
              '<div id="selectsearch" class="selectsearch"><div id="floaticon"></div><div id="list"></div><div id="searchlist"><ul></ul><ul><li data-url="http://option/">Options...</li></ul></div></div>'
            );
            ...
            $("body").mouseup(function (g) {
              ...
              if (!(d == "list" || d == "floaticon")) {
                ...
                var f = g.clientY - 40 < 0 ? g.clientY + sh + 10 : g.clientY + sh + 10,
                  k = window.getSelection().toString();
...
chrome.extension.sendRequest(
  ...
  function (a) {
    if (!(HOST == "docs.google.com" || HOST == "mail.google.com"))
      if (a.option.length > 0) {
        ...
        if (a.bookmarks == "true" || a.history == "true")
          try {
            buildbookmartbar();
          } catch (c) {}
      }
...
function buildbookmartbar() {
  ...
  if (a && HOST != "www.bing.com") {
    ...
    j.id = "searchdiv";
    var g = document.getElementById("rhs"),
      d = function (f, k, h) {
        for (var l = f.length > 8 ? 8 : f.length, q = document.createDocumentFragment(), m = 0; m < l; m++) {
          var n;
          if (f[m].title == "") n = f[m].url;
          else {
            n = f[m].title;
            var o = c.replace(/[\.\$\^\{\[\(\|\)\*\+\?\\]/gi, "\\$1");
            o = RegExp("(" + o + ")", "ig");
      ...
      chrome.extension.sendRequest(
        ...
        function (f) {
          if (f.bookmarkItems.length >= 1 || f.historyItems.length >= 1) {
            if (g) {
              var k = g.firstChild;
              if ($("#searchdiv").length > 0) return;
              else g.insertBefore(j, k);
            }
            ...
            $("#searchdiv-bookmark-container")
              .find(".viewAllLink")
              .text("View all " + f.bookmarkItems.length + " " + k)
              .on("click", function () {
                chrome.extension.sendRequest({
                  action: "newTab",
                  url: "chrome://bookmarks/#q=" + c,
                });
              });
            $("#searchdiv-history-container")
              .find(".viewAllLink")
              .text("View all " + f.historyItems.length + " " + h)
              .on("click", function () {
                chrome.extension.sendRequest({
                  action: "newTab",
            ...
            $(".searchdiv-close-action").on("click", "li", function (l) {
              l = l.target;
              if ($(l).hasClass("searchdiv-close-t"))
                if (l.parentNode.id == "bookmark-close-action") $("#searchdiv-bookmark-box").hide();
                else l.parentNode.id == "history-close-action" && $("#searchdiv-history-box").hide();
              else if ($(l).hasClass("searchdiv-close-f"))
                if (confirm("Want to disable this feature? You can enable it in the Search All extension options again.") == true)
                  if (l.parentNode.id == "bookmark-close-action") {
                    $("#searchdiv-bookmark-box").hide();
                    chrome.extension.sendRequest({
                      action: "closeBookmarkSearch",
                    });
                  } else {
                    if (l.parentNode.id == "history-close-action") {
                      $("#searchdiv-history-box").hide();
                      chrome.extension.sendRequest({
                        action: "closeHistorySearch",
                      });
                    }
                  }
                else return;
    ...
    function (c) {
      ...
      if (!document.getElementById("sa-admanage-box")) {
        ...
        for (var g = document.createElement("ol"), d = 0; d < e; d++) {
          var f = myReplace(a, c[d].title["#cdata"]),
            k = myReplace(a, c[d].descr["#cdata"]),
            h = myReplace(a, c[d].host["#cdata"]);
          g.innerHTML +=
            '<li class="Admanage-li"><h3><a target="_blank" href="' +
            c[d].url["#cdata"] +
            '">' +
            f +
            "</a></h3><div><cite>" +
...
function getAffiate() {
  Array.prototype.slice.call(document.getElementsByTagName("a")).forEach(function (a, c, e) {
    if (/www.amazon.com/.test(a.href) && !/(&|\?)tag=([^&]*)(&|$)/.test(a.href))
      e[c].href += /\?.*=/.test(a.href) ? "&tag=diigo0c-20" : "?tag=diigo0c-20";
  });
}
