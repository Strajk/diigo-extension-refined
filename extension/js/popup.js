...
$(document).ready(function () {
  function n(a) {
    ...
    if (/^https?:\/\/chrome\.google\.com\/(extensions|webstore)/i.test(a)) return false;
    return true;
  }
  ...
  function x(a, b) {
    if (y !== true) {
      ...
      if (a.generated == true) {
        ...
        if (c.saved == true) {
          $("#remove").show();
          $("#remove").attr("data-gtooltip", "Saved " + c.datetime + ". Click to remove");
          ...
          $("#cached")
            ...
            .bind("click", function () {
              getSelectedTab(function (d) {
                var f = "https://www.diigo.com/cached?url=" + encodeURIComponent(d.url);
                chrome.tabs.create({
                  url: f,
                  index: d.index + 1,
                });
        ...
        if (c != undefined) {
          console.log(b);
          c.saved != true && b ? $("#diigobm-title-input").val(b).trigger("input") : $("#diigobm-title-input").val(c.title).trigger("input");
  ...
  function s() {
    ...
    $("#tag-cloud-container")
      ...
      .each(function () {
        var c = $(this),
          d = c.text();
        $.inArray(d, b) != -1 ? c.addClass("selected") : c.removeClass("selected");
      });
  ...
  function H(a) {
    if (a.length == 0) {
      ...
      $("#tag-cloud-container").on("click", "a", function (k) {
        /Diigo\-Bookmark\-Tag\-item/.test(k.target.className) && z($(this).text());
      });
      s();
    }
  }
  ...
  function u(a) {
    ...
    if (a.length) {
      ...
      forEach(a, function (d) {
        ...
        $('<div class="menu-item" data-id="' + d.id + '">' + f + "</div>").appendTo(c);
      });
    }
    if (b.length) {
  ...
  function A(a, b) {
      ...
      forEach(d, function (e) {
        ...
        $('<div class="menu-item" data-id="' + e.name + '">' + h + "</div>")
          .on("click", function () {
            console.log("Click");
            var i = e.name;
            if (popCtx.groupTagsDict[i] != undefined) q("group", popCtx.groupTagsDict[i]);
            else {
    ...
    var g = $("#diigobm-group").find("select").empty().unbind().removeClass("processing");
    if (d.length > 0) {
      g.append(
        Utils.dom.buildOne(
          "option",
    ...
    g.change(function () {
      ...
      if (e != 0 && e != -1 && e != -2 && e != -3) {
        ...
        console.log(popCtx);
        if (popCtx.groupTagsDict[e] != undefined) q("group", popCtx.groupTagsDict[e]);
        else {
          $("#diigobm-group-tag").show();
          chrome.tabs.getSelected(null, function (h) {
            chrome.tabs.sendMessage(h.id, {
  ...
  function I(a, b) {
    ...
    $("#remove").attr("data-gtooltip", "Saved " + c.datetime + ". Click to remove");
    if (c != undefined) {
      c.saved != true && b != undefined ? $("#diigobm-title-input").val(b) : $("#diigobm-title-input").val(c.title);
      m($("#diigobm-title-input"), "change");
      $("#diigobm-url-input").val(c.url);
    } else
    ...
    c.mode == 2 && $("#op-private").prop("checked", true);
    if (c.saved == false) {
      localStorage["prefs.bookmark.privateByDefault"] == "true" && $("#op-private").prop("checked", true);
      localStorage["prefs.bookmark.unreadByDefault"] == "true" && $("#op-readlater").prop("checked", true);
    }
    c.unread == true && $("#op-readlater").prop("checked", true);
    ...
    if (localStorage.openedOutlinerId && localStorage["prefs.outlinerIcon"] == "true") {
  ...
  function J(a, b) {
    ...
    c._private === true || b == 2 ? $("#op-private").prop("checked", true) : $("#op-private").prop("checked", false);
    c.unread === true && $("#op-readlater").prop("checked", true);
    ...
    if (c.outliner) {
      ...
      f.find(".content").text(d).attr("data-id", c.outliner).end().addClass("inputed");
    }
    if (c.group) {
      f = $("#diigobm-group");
      d = f.find(".menu-item[data-id=" + c.group + "]").text();
      f.find(".content").text(d).attr("data-id", c.group).end().addClass("inputed");
    }
  }
  ...
  function v(a) {
    ...
    if (a) {
      ...
      chrome.storage.local.get("isShowResearchTip", function (e) {
        console.log(e);
        if (typeof e.isShowResearchTip === "undefined" || e.isShowResearchTip != false) $("#research-mode-notification").show();
      });
    ...
    $("#link-icon").on("click", function () {
      ...
      else e.hasClass("unfold") ? e.addClass("fold").removeClass("unfold") : e.addClass("unfold").removeClass("fold");
    });
    ...
    $("#bm-main").on("keydown", function (e) {
      if (e.keyCode == 13) e.target.id == "diigobm-tag-input" && !$("#diigolet-ac").is(":visible") && r();
    ...
    $("#remove").Gtooltip();
    a = [];
    var c = popCtx.myTagsWithCount ? popCtx.myTagsWithCount : j.GlobalData.myTagsWithCount;
    c.sort(function (e, h) {
      return e.count <= h.count ? 1 : -1;
    });
    ...
    $("#diigobm-list-addBtn").on("click", function () {
      ...
      if (!$(this).parent().hasClass("processing"))
        if (j.GlobalData.permissions.createOutliner) {
          ...
          } else if ($.inArray(e, i) !== -1) h.show().find("span").text("Name existed !");
          else {
    ...
    $("#diigobm-recent-tag,#diigobm-recommend-tag,#diigobm-group-tag").on("click", "div", function (e) {
    ...
    chrome.browserAction.getBadgeText(
      ...
      function (b) {
        if (b == "New") {
          ...
          localStorage.newClickVersion = j.newClickVersion;
          window.close();
        }
      }
    );
  ...
  if (localStorage["prefs.shortcutOpenOutlinerSidebar"] == "true") {
    ...
    var w = "";
    if (localStorage["prefs.shortcutOpenOutlinerSidebarCtrl"] == "true") w += "Ctrl + ";
    if (localStorage["prefs.shortcutOpenOutlinerSidebarAlt"] == "true") w += "Alt + ";
  ...
  chrome.cookies.get(
    ...
    function (a) {
      if (a) {
        ...
        permissions.servicePlanName && permissions.servicePlanName == "Free" && $("#userInfo").find(".upgrade").show();
            ...
            if (c === "tutorial")
              ...
              switch (c) {
                case "home":
                  ...
                  break;
                case "rating":
                  b = "https://chrome.google.com/webstore/detail/diigo-web-collector-captu/pnhplgjpclknigjpccbcnmicgcieojbh/reviews";
                  break;
                case "unread":
                  b = "https://www.diigo.com/user/" + p + "?type=bookmark&read=no";
                  break;
                case "my-outliner":
                ...
                case "option":
                  ...
                  break;
                case "changelog":
                  b = "https://blog.diigo.com/2017/08/22/introduce-a-new-way-to-retain-knowledge-from-books/";
                  break;
                case "sign-out-btn":
                ...
                case "upgrade":
                  ...
                  googleEvent("Chrome extension", "Click upgrade from main menu", "Name");
                  j.sendAnalysisData({
        ...
        getSelectedTab(function (b) {
          var c = b.url,
            d = c.match(/https?:\/\/*\/*/gi);
          console.log(n(c));
          if (d == null || !n(c)) $("ul").find("li").not("#annotate-pdf").addClass("disabled").off("click");
          else {
            ...
            chrome.tabs.sendMessage(
              ...
              function (f) {
                f && f.opened && $("#toggle-outliner").find(".content").text("Close outliner sidebar");
              }
            );
  ...
  chrome.runtime.onMessage.addListener(function (a, b) {
    if (a.name == "isSaved") {
      var c = a.data;
      if (c.saved === true) {
        $("#save-bookmark").find("span").text("Edit Bookmark").end().addClass("saved");
        $("#cached")
          .show()
          .bind("click", function () {
            getSelectedTab(function (f) {
              var g = "https://www.diigo.com/cached?url=" + encodeURIComponent(f.url);
              chrome.tabs.create({
                url: g,
                index: f.index + 1,
              });
    ...
    } else a.name == "listCreateFailureAndUpdateLists" && $("#diigobm-list-addBtn").parent().removeClass("processing");
  });
      ...
      case "annotate-pdf":
        chrome.tabs.getSelected(null, function (c) {
          c.url.substring(c.url.length - 4) == ".pdf"
            ? chrome.tabs.create({
                url: "/pdf/viewer.html?file=" + c.url,
              })
            : chrome.tabs.create({
                url: "/pdf/viewer.html",
              });
        ...
        window.addEventListener(
          ...
          function (c) {
            ...
            if (d == "openUrl") {
              ...
              d = c.data.tag.indexOf(" ") != -1 ? '#"' + c.data.tag + '"' : "#" + c.data.tag;
              $("#search-input").val(d);
    ...
    window.addEventListener(
      ...
      function (a) {
        ...
        if (b == "openUrl") {
          ...
          b = a.data.tag.indexOf(" ") != -1 ? '#"' + a.data.tag + '"' : "#" + a.data.tag;
          $("#search-input").val(b);
          $("#search-btn").trigger("click");
        }
      },
  ...
  $("#tag-edit").on("click", function () {
    chrome.tabs.create({
      url: "https://www.diigo.com/cloud/" + p,
    });
  });
