...
function log(a, c) {
  try {
    ...
    b > -1 && d > -1 && b >= d && window.console && console.log.apply(console, a);
  } catch (e) {}
}
...
function arrayUnique(a) {
  a = a.concat();
  for (var c = 0; c < a.length; ++c) for (var b = c + 1; b < a.length; ++b) a[c] === a[b] && a.splice(b--, 1);
  return a;
}
...
function extend() {
  for (var a = [].slice.call(arguments), c = a.shift(), b = 0, d = a.length, e; (e = a[b]), b < d; b++) for (var f in e) c[f] = e[f];
  return c;
}
...
function forEach(a, c, b) {
  if (a.length !== undefined) for (var d = 0, e = a.length; d < e; d++) c.call(b, a[d], d);
  else for (d in a) c.call(b, a[d], d);
}

function filter(a, c, b) {
  for (var d = [], e = 0, f = a.length; e < f; e++) c.call(b, a[e], e) && d.push(a[e]);
  return d;
}

function map(a, c, b) {
  for (var d = a.length, e = Array(d), f = 0; f < d; f++) e[f] = c.call(b, a[f], f);
  return e;
}
...
function findIndex(a, c, b) {
  ...
  b = b;
  for (var e = a.length, f; (f = a[b]), b < e; b++) if (d ? c(f) : f == c) return b;
  return -1;
}
...
function reverse(a) {
  for (var c = Array(a.length), b = a.length - 1, d = 0; b >= 0; b--, d++) c[d] = a[b];
  return c;
}
...
var Utils = {
  ...
  IEventDispatcher = {
    mixin: function (a) {
      extendThese(a, this, $w("addEventListener removeEventListener fireEvent _resetEvents"));
      a._resetEvents();
    },
    ...
    fireEvent: function (a, c) {
      if (!this._supressEvents) {
        debug("[event]", a);
        var b = this._events[a];
        if (b) for (var d = 0, e, f = b.length; (e = b[d]), d < f; d++) (typeof e == "function" ? e : e["on" + a]).apply(e, c);
      }
    },
  };
