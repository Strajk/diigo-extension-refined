var TwitterOAuth = {
  ...
  onTabUpdated: function (a, b, c) {
    if (!(!this.oauthPinTab || this.oauthPinTab.id != a || b.status != "complete" || !/twitter.com\/oauth\/authorize$/.test(c.url))) {
  ...
  getAccessToken: function (a, b) {
    ...
    if (a) {
      ...
      chrome.tabs.update(D.switchToTabIdAfterTwitterOauth, {
        selected: true,
      });
      c.makeRequest({
        url: "access_token",
        ...
        successCallback: function (e) {
          ...
          if (
            "oauth_token oauth_token_secret user_id screen_name".split(" ").every(function (d) {
              return !!f[d];
            })
          ) {
  ...
  loadOauthInfo: function () {
    ...
    if (a)
      try {
        ...
        if (
          !"oauth_token oauth_token_secret user_id screen_name".split(" ").every(function (c) {
            return !!a[c];
          })
        ) {
