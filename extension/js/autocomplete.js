var AutoComplete = function (x, l) {
  ...
  function y(a) {
    if (g.mode == "multiple") {
      ...
      u = a.substring(0, b[0]) == "" ? a.substring(0, b[0]) : a.substring(0, b[0]) + " ";
      if (c.length > 0) {
    ...
    for (var i; (i = g.data[c]), c < g.data.length; c++)
      if (i.toLowerCase().indexOf(a) == 0) d.indexOf("#") == 0 ? b.push("#" + i) : b.push(i);
      else if (i.toLowerCase().indexOf(a) != 0 && i.toLowerCase().indexOf(a) != -1) d.indexOf("#") == 0 ? h.push("#" + i) : h.push(i);
    h.sort(function (z, A) {
  ...
  function v(a, b) {
    if (b && b.length > 0) {
      ...
      var c = (function (d) {
        for (var h = d.offsetLeft || 0, i = d.offsetTop || 0; (d = d.offsetParent); ) {
          h += d.offsetLeft;
          i += d.offsetTop;
        }
      ...
      e.find("ul")
        ...
        .end()
        .css({
          top: g.moveToSelect ? c.y + f[0].offsetHeight - 4 : c.y + f[0].offsetHeight + 3,
          left: c.x,
        })
        .show();
      e.find("li")
  ...
  if (typeof g.multipleSeparator == "string") g.multipleSeparator = g.multipleSeparator.split("");
