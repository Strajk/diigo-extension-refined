...
var SignInManager = (D.SignInManager = {
        ...
        },
        {
          ...
          complete: function (a) {
            if (a.resp) {
              ...
              if (
                typeof localStorage["prefs.directlyShowSearchResults"] === "undefined" &&
                ...
                } else localStorage["prefs.directlyShowSearchResults"] = "false";
              var b = a.resp.result.permissions.autoShowAnnotation;
              if (typeof b !== "undefined" && b == false)
                if (localStorage.free_trial_count) {
                  if (localStorage.free_trial_count && localStorage.free_trial_count >= 100) {
                    Prefs.set({
  ...
  Preloader = (D.Preloader = {
    ...
    hasPrivateAnnotation: function (a, b) {
      var c = 0,
        d;
      for (d = 0; d < a.length; d++) if (a[d].user == b || a[d].onlyInGroup == true) c++;
...
function isURLApplicable(a, b) {
  ...
  if (/\.?diigo\./i.test(c.host))
    ...
    else if (/(player|slides?)\.diigo\./i.test(c.host) && /^\/(list|feed)/i.test(d)) return false;
    ...
    e.invoke(a, b, {
      ...
      complete: function (f) {
        ...
        if (f)
          if (f.resp.code == 1) {
            if (c != "saveLink") {
              var g = "img/" + (a == "bm_saveBookmark" || a == "pdf_createItem" ? "logo-19-bookmarked.png" : "logo-19.png");
...
var Messenger = {
  ...
  handlers: {
    ...
    newtip: function () {
      ...
      window.open("https://www.diigo.com/extension/chrome/new.html?v=" + D.version);
    },
    annotateAfterSaving: function (a) {
      localStorage["prefs.annotateAfterSaving"] = a.data == true ? "true" : "false";
    },
    capture: function (a) {
    ...
    captureVideo: function (a, b, c) {
      ...
      chrome.tabs.captureVisibleTab(
        ...
        function (e) {
          ...
          h.src = e;
          h.onload = function () {
            g.drawImage(h, d.left, d.top, d.width, d.height, 0, 0, d.width, d.height);
            var i = f.toDataURL();
            console.log(i);
            c(i);
            $(f).remove();
      ...
      getSelectedTab(function (a) {
        ...
        a.openerTabId &&
          chrome.tabs.get(a.openerTabId, function (b) {
            var c,
              d = RegExp("https?://(?:.+\\.)?google\\.[a-z]{2,3}(?:\\.[a-z]{2})?(?:\\:[0-9]+)?/(?:advanced_)?search.*(([?&]q=)([^&]+))"),
              e = RegExp("https?://(?:.+\\.)?google\\.[a-z]{2,3}(?:\\.[a-z]{2})?(?:\\:[0-9]+)?/(?:webhp[^#]*)?(#((?:.+&)?q=)([^&]+)(.*)&fp=[^&]+)");
    ...
    getSearchPref: function (a, b, c) {
      c(localStorage["prefs.directlyShowSearchResults"] == "true" && GlobalData.permissions.autoShowAnnotation);
    },
    openPdf: function (a) {
      a.url.substring(a.url.length - 4) == ".pdf"
        ? chrome.tabs.create({
            url: "/pdf/viewer.html?file=" + a.url,
          })
        : chrome.tabs.create({
            url: "/pdf/viewer.html",
          });
    ...
    importKindle: function (a, b) {
      $.ajax({
        ...
        data: {
          toolbar_data: JSON.stringify(a.data),
        },
        success: function () {
...
function showToolbar(a, b) {
  if (typeof b != "undefined") {
    ...
    getSelectedTab(function (c) {
      if (/^https?:\/\/chrome\.google\.com\/(extensions|webstore)/i.test(c.url)) alert(chrome.i18n.getMessage("alertGallery"));
...
function init() {
  ...
  if (localStorage.version != D.version)
    if (localStorage.version && Upgrader.isVersionBefore(localStorage.version, "3.3.30")) {
  ...
  chrome.runtime.onMessage.addListener(function (a, b, c) {
    ...
    if (!(a.name == "sendCtx" || a.name == "saved" || a.name == "sendRetags" || a.name == "updateGroups" || a.name == "updateLists")) {
      Messenger.onMessage(a, b, c);
      return true;
    }
  });
  chrome.tabs.onUpdated.addListener(function (a, b, c) {
    b.url == "https://www.diigo.com/chrome_diigo_extension_done" && chrome.tabs.remove(c.id);
    ...
    if (b.status == "complete" && /diigo\.(com|cn)\/images\/diigo-logo.png#SIGNED_IN/i.test(c.url)) {
      debug("[Sign in tab opened by extension detected!]", a, b, c);
      chrome.tabs.remove(a);
      chrome.tabs.update(D.activateTheTabIdAfterSignIn, {
        selected: true,
      });
    ...
    SignInManager.checkSignInCookie(function () {
      Prefs.get("prefs.autoloadBookmarkStatus") === "true" && Preloader.onTabUpdated(a, b, c);
...
var Prefs = {
    ...
    _notifyChanges: function (a) {
      for (var b = 0, c = this._observers.length; b < c; b++) this._observers[b].call(null, a);
    },
  ...
  EventMachine = (D.EventMachine = {
    ...
    needToReload: function () {
      if ((new Date().getTime() - this._lastReloadTime.getTime()) / 1e3 / 60 > 15) {
    ...
    if (c.isInGroup) {
      ...
      e =
        a == "user_signIn"
          ...
          : evalTpl(b + "/chappai/pv=#{pv}/ct=tb/cv=#{cv}/user=#{user}/cmd=#{cmd}/", {
              pv: 13,
              cv: D.version,
              user: e,
              cmd: a,
  ...
  performInvocation: function (a, b, c, d) {
    ...
    e.xhr = $.ajax({
      ...
      complete: function (f) {
        ...
        if (d == "upload_file" || d == "bm_saveBookmark" || d == "list_create") {
          e.status = f.status;
          e.cmd = d;
        }
        if ((e.resp = g)) {
...
var Upgrader = {
  ...
  compareVersion: function (a, b) {
    ...
    a = c(a);
    b = c(b);
    for (var d = 0; d < a.length && d < b.length; d++) if (a[d] != b[d]) return a[d] > b[d] ? 1 : -1;
                ...
                if (g)
                  (username = g.value.split("-.-")[1]) &&
                    chrome.tabs.update(f.id, {
                      url: "https://www.diigo.com/user/" + username + "?from=extension",
                    });
              }
            );
          }
