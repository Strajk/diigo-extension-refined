...
$("#tag-edit").on("click", function () {
  chrome.tabs.create({
    url: "https://www.diigo.com/cloud/" + username,
  });
});
$("#remove").on("click", function () {
  sendMessage({
    type: "qdeletebookmark",
  });
});
$("#show-url")
  .on("click", function () {
...
function updateTagStatus() {
  ...
  $("#tag-cloud-container")
    ...
    .each(function () {
      var d = $(this),
        e = d.text();
      $.inArray(e, b) != -1 ? d.addClass("selected") : d.removeClass("selected");
    });
        ...
        $("#cached")
          ...
          .bind("click", function () {
            getSelectedTab(function (d) {
              var e = "https://www.diigo.com/cached?url=" + encodeURIComponent(d.url);
              chrome.tabs.create({
                url: e,
                index: d.index + 1,
              });
...
function initBookmarkWindow(a) {
  ...
  if (a) {
    ...
    chrome.storage.local.get("isShowResearchTip", function (c) {
      if (typeof c.isShowResearchTip === "undefined" || c.isShowResearchTip != false) $("#research-mode-notification").show();
    });
  ...
  $("#link-icon").on("click", function () {
    ...
    else c.hasClass("unfold") ? c.addClass("fold").removeClass("unfold") : c.addClass("unfold").removeClass("fold");
  });
  ...
  $("#bm-main").on("keydown", function (c) {
    if (c.keyCode == 13) c.target.id == "diigobm-tag-input" && !$("#diigolet-ac").is(":visible") && formToObject();
  ...
  a = [];
  var d = popCtx.myTagsWithCount ? popCtx.myTagsWithCount : GlobalData.myTagsWithCount;
  d.sort(function (c, h) {
    return c.count <= h.count ? 1 : -1;
  });
  ...
  $("#diigobm-list-addBtn").on("click", function () {
    if (!$(this).parent().hasClass("processing"))
      if (GlobalData.permissions.createOutliner) {
        ...
        } else if ($.inArray(c, i) !== -1) h.show().find("span").text("Name existed !");
        else {
...
function objectToform(a) {
  ...
  b.mode == 2 && $("#op-private").prop("checked", true);
  b.saved == false && localStorage["prefs.bookmark.privateByDefault"] == "true" && $("#op-private").prop("checked", true);
  if (b.unread == true || (b.saved == false && localStorage["prefs.bookmark.unreadByDefault"] == "true")) $("#op-readlater").prop("checked", true);
...
function updateLists(a, b) {
  ...
  if (e.length) {
    ...
    forEach(e, function (c) {
      ...
      $('<div class="menu-item" data-id="' + c.id + '">' + h + "</div>").appendTo(g);
    });
  }
  if (d.length) {
  ...
  var f = $("#diigobm-list").find("select").empty().unbind().removeClass("processing");
  f.append(
    Utils.dom.buildOne(
      "option",
      {
...
function updateGroups(a, b) {
  ...
  var f = $("#diigobm-group").find("select").empty().unbind().removeClass("processing");
  if (e.length > 0) {
    f.append(
      Utils.dom.buildOne(
        "option",
  ...
  f.change(function () {
    ...
    if (c != 0 && c != -1 && c != -2 && c != -3) {
      ...
      if (popCtx.groupTagsDict[c] != undefined) showTags("group", popCtx.groupTagsDict[c]);
      else {
        $("#diigobm-group-tag").show();
        chrome.tabs.getSelected(null, function (h) {
          chrome.tabs.sendMessage(h.id, {
...
function fillReasearchModeData(a, b) {
  ...
  d._private === true || b == 2 ? $("#op-private").prop("checked", true) : $("#op-private").prop("checked", false);
  ...
  if (d.outliner) {
    ...
    g.find(".content").text(e).attr("data-id", d.outliner).end().addClass("inputed");
  }
  if (d.group) {
    g = $("#diigobm-group");
    e = g.find(".menu-item[data-id=" + d.group + "]").text();
    g.find(".content").text(e).attr("data-id", d.group).end().addClass("inputed");
  }
}
...
function buildTagCloud(a) {
  if (a.length == 0) {
    ...
    $("#tag-cloud-container").on("click", "a", function (j) {
      /Diigo\-Bookmark\-Tag\-item/.test(j.target.className) && toggleTag($(this).text());
    });
    updateTagStatus();
  }
}
