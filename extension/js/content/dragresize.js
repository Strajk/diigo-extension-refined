...
DragResize.prototype.mouseMove = function (a) {
  with (this) {
    ...
    mouseX = a.pageX || a.clientX + document.documentElement.scrollLeft;
    mouseY = a.pageY || a.clientY + document.documentElement.scrollTop;
    var b = mouseX - lastMouseX + mOffX,
      c = mouseY - lastMouseY + mOffY;
    mOffX = mOffY = 0;
    lastMouseX = mouseX;
    lastMouseY = mouseY;
    if (!handle) return true;
    var d = false;
    if (this.resizeHandleDrag && this.resizeHandleDrag(b, c)) d = true;
    else {
      var e = b,
        f = c;
      if (elmX + e < minLeft) mOffX = e - (b = minLeft - elmX);
      else if (elmX + elmW + e > maxLeft) mOffX = e - (b = maxLeft - elmX - elmW);
      if (elmY + f < minTop) mOffY = f - (c = minTop - elmY);
      else if (elmY + elmH + f > maxTop) mOffY = f - (c = maxTop - elmY - elmH);
      elmX += b;
      elmY += c;
    }
    with (element.style) {
...
DragResize.prototype.resizeHandleDrag = function (a, b) {
  with (this) {
    var c = handle && handle.className && handle.className.match(RegExp(myName + "-([tmblr]{2})")) ? RegExp.$1 : "",
      d = b,
      e = a,
      f = false;
    if (c.indexOf("t") >= 0) {
      rs = 1;
      if (elmH - d < minHeight) mOffY = d - (b = elmH - minHeight);
      else if (elmY + d < minTop) mOffY = d - (b = minTop - elmY);
      elmY += b;
      elmH -= b;
      f = true;
    }
    if (c.indexOf("b") >= 0) {
      rs = 1;
      if (elmH + d < minHeight) mOffY = d - (b = minHeight - elmH);
      else if (elmY + elmH + d > maxTop) mOffY = d - (b = maxTop - elmY - elmH);
      elmH += b;
      f = true;
    }
    if (c.indexOf("l") >= 0) {
      rs = 1;
      if (elmW - e < minWidth) mOffX = e - (a = elmW - minWidth);
      else if (elmX + e < minLeft) mOffX = e - (a = minLeft - elmX);
      elmX += a;
      elmW -= a;
      f = true;
    }
    if (c.indexOf("r") >= 0) {
      rs = 1;
      if (elmW + e < minWidth) mOffX = e - (a = minWidth - elmW);
      else if (elmX + elmW + e > maxLeft) mOffX = e - (a = maxLeft - elmX - elmW);
      elmW += a;
      f = true;
    }
    return f;
